# -*- coding: utf-8 -*-
"""
Created on Tue Feb 12 21:57:59 2019
...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    more_temperatures = np.array(more_temperatures).tolist()
  
     
    temperatures = temperatures + more_temperatures
     
    for i in range(1,11):
        temperatures = temperatures + [iot_sensor_points[i]]
    temperatures = temperatures + [a]+[b]+[c]+[d]+[e]     
    temperatures = sorted(temperatures, reverse = True)
    
    for i in range(1,11):
        samples = samples + [temperatures[(5*i)-1]]
...
# 6. An EXIT menu option should be added at the end of the displayed list of menu options allowing the user to exit selecting a menu causing the display_menu() function to return back the number of the 
#last menu option chosen prior to exit and also return the length of menu (the display_menu() function returns two values)
# 7. The menu options should repeatedly display after each selection until user selects EXIT

# ------ Plmenu = ['Buy Bitcoin','Buy Ethereum','Sell Bitcoin','Sell Ethereum'ace code below here \/ \/ \/ ------
menu = ['Buy Bitcoin','Buy Ethereum','Sell Bitcoin','Sell Ethereum']
menu = ('Buy Bitcoin','Buy Ethereum','Sell Bitcoin','Sell Ethereum')

def display_menu(menu):
        len_menu = 0
        last_option_selected = 0
        if type(menu) is tuple:
            ...
            while ans:
                ...
                if ans=="1": 
                  ...
                  print("\n EXIT \n")
                  return int(last_option_selected),len_menu
                elif ans !="":
                  print("\n Not Valid Choice Try again"+"\n") 
                last_option_selected = ans
        else:
            return -1,len(menu)
...
def exercise10(sentence):
    ...
    for i in range(len_sen-1,-1,-1):
        if sentence[i].islower():
            reversed1 = reversed1 + sentence[i].upper()
        else:
            reversed1 = reversed1 + sentence[i].lower()
    reversed1 = reversed1.replace(" ", "_")
