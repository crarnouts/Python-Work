...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    # ------ Place code below here \/ \/ \/ ------
    more_temperatures = np.random.randint(300, 400, size=25) # remove once function is complete
    more_temperatures = np.array(more_temperatures).tolist()
    iot_sensor_points = {1: 801, 2: 644, 3: 991, 4: 721,   ##remove once function is complete
    ...
    for i in range(1,int(len(temperatures)/5)):
        samples = samples + [temperatures[5*i]]
