# -*- coding: utf-8 -*-
"""
Created on Sun Mar 10 18:37:19 2019
...
'''
coreychain = PandasChain("Corey")
coreychain.display_chain()

coreychain.add_transaction("Bob","Alice",53)
coreychain.add_transaction("Corey","Lyss",55)
...
class PandasChain:
    ...
    def __init__(self, name): 
        ...
        self.__current_block = Block(self.seq_id,self.__prev_hash)
    ...
    def add_transaction(self,s,r,v): 
        # this is an add transaction method in the pandachain class that really is just looking if a transaction can be added in the specific block and if it can it is calling the method through that block object
...
block.add_transaction("Corey","Lyss",55)
block.add_transaction("Bob","Alice",51)
...
class Block:
    ...
    def __init__(self,seq_id,prev_hash): 
        ...
        self.__transactions = pd.DataFrame(columns=['Timestamp','Sender','Receiver','Value','TxHash']) # should be able to replace this with the self.__colnames object
    ...
    def display_header(self): 
        print("seq_id:" + str(self.__seq_id)+" status:" + str(self.__status) + " Block_hash:" 
              + str(self.__block_hash) + " Previous_Block_Hash:" + str(self.__prev_hash) + 
              " Merkle_Hash:" + str(self.get_simple_merkle_root()) + " Number of Transactions:" + str(len(self.__transactions.index))) 
    ...
    def add_transaction(self,s,r,v): 
        ...
        self.__v = v
        ts = dt.datetime.now() 
        tx_hash = hashlib.sha256(str(self.__s + self.__r + str(self.__v) +str(ts)).encode('utf-8')).hexdigest()
        self.__transactions.loc[len(self.__transactions.index)]= [ts,self.__s,self.__r,self.__v,tx_hash] ## this takes into consideration the amount of rows that are already in the dataframe and then appends a new row based off the args provided
        #print(self.__transactions.to_string())
        #new_transaction = # Create DataFrame with transaction data (a DataFrame with only 1 row), I don't know if I need this, this was his way of doing it 
    ...
    def set_status(self,status):
        self.__status = status #use the pandas chain to set this
    ...
    def get_simple_merkle_root(self): 
        string = ''
        for i in range(0,len(self.__transactions.index)):
            string = string + str(self.__transactions.iloc[i]['TxHash'])
        return hashlib.sha256(string.encode('utf-8')).hexdigest()
    
    def get_values(self):
        y = []
        for i in range(len(self.__transactions.index)):
            y.append(self.__transactions.iloc[i]['Value'])
        return y
...
pandas_chain.add_transaction("e","Alice",51)
pandas_chain.add_transaction("Bob","Alice",52)
pandas_chain.add_transaction("Bob","f",53)
pandas_chain.add_transaction("Bob","Alice",53)
pandas_chain.add_transaction("Bob","Alice",53)
pandas_chain.add_transaction("Bob","Alice",53)
pandas_chain.add_transaction("g","Alice",53)
pandas_chain.add_transaction("Bob","Alice",53)
pandas_chain.add_transaction("Bob","Alice",53)
pandas_chain.add_transaction("Bob","Alice",53)
pandas_chain.add_transaction("Bob","Alice",50)
pandas_chain.add_transaction("c","Alice",51)
pandas_chain.add_transaction("Bob","Alice",52)
pandas_chain.add_transaction("Bob","Alice",53)
pandas_chain.add_transaction("Bob","d",53)
pandas_chain.add_transaction("Bob","Alice",53)
pandas_chain.add_transaction("Bob","Alice",53)
pandas_chain.add_transaction("Bob","Alice",53)
pandas_chain.add_transaction("Bob","Alice",53)
