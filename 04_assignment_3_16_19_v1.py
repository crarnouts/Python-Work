...
class PandasChain:
    ...
    def __init__(self, name): 
        ...
        self.__df = pd.DataFrame(columns=['Timestamp','Sender','Receiver','Value','TxHash'])
    ...
    def add_transaction(self,s,r,v): 
        ...
        self.__r = r
        self.__v = v
        self.__timestamp = dt.datetime.now()
        self.__TxHash = hashlib.sha256(str(self.__s + self.__r + str(self.__v) +str(self.__timestamp)).encode('utf-8')).hexdigest()
        self.__df.loc[len(self.__df.index)]= [self.__timestamp,self.__s,self.__r,self.__v,self.__TxHash] ## this takes into consideration the amount of rows that are already in the dataframe and then appends a new row based off the args provided
