# -*- coding: utf-8 -*-
"""
Created on Tue Apr  9 19:25:50 2019
...
def exercise01():
    ...
    df = pd.DataFrame([[1,2,3,4],[5,6,7,8],[7,8,9,10]],columns=['0','1','2','3'])
...
def exercise05(n):
    ...
    zeros = np.array([[0 for col in range(n)] for row in range(n)])
...
def exercise06(n):
    ...
    ones = np.array([[1 for col in range(n)] for row in range(n)])
...
def exercise13(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    s = pd.Series(np.random.randint(0,n,size=(n),dtype ='l'),index=pd.date_range('2010-01-01', freq='D', periods=n))
    ts = df.cumsum()
    ts.plot()
