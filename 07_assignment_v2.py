...
def exercise01():
    ...
    # ------ Place code below here \/ \/ \/ ------
    df_first_five_rows = pd.DataFrame(data= np.c_[iris['data'], iris['target']],
                     columns= iris['feature_names'] + ['target'])[0:5]
...
def exercise03(neighbors,split):
    ...
    X = np.array(df.drop(['target'],1))
    y = np.array(df['target'])
    #X_train, X_test, y_train, y_test = tts(iris.data, iris.target, test_size=split, random_state=random_state)
