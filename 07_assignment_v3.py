# -*- coding: utf-8 -*-
"""
Created on Sat Apr 27 13:41:53 2019
...
import plotly
plotly.tools.set_credentials_file(username='crarnouts', api_key='6tMWckpaL9T0Mrp4joNV')
...
def exercise01():
    ...
    # ------ Place code below here \/ \/ \/ ------
    df_first_five_rows = pd.DataFrame(data= np.c_[iris['data']],
                     columns= iris['feature_names'])[0:5]
...
def exercise02(new_observations):
    ...
    encoder.fit(labels)
    iris_predictions = clf.predict(new_observations)
    iris_predictions = iris_predictions.astype(int)
    iris_predictions = encoder.inverse_transform(iris_predictions)
...
def exercise04():
    ...
    # ------ Place code below here \/ \/ \/ ------
    results = []
    knn = np.array(range(1,30))
    for k in knn:
        results.append(exercise03(k,.3))
    ...
    plotly_overfit_underfit_curve_url = 'https://plot.ly/~crarnouts/24'
...
def exercise05():
    ...
    average_medv = df_target.values.mean()
...
def exercise06():
    ...
    plt.scatter(df['PTRATIO'],df_target, alpha=0.5)
    plt.title('Pupil to Teacher Ratio Compared to Median Home Value')
    plt.xlabel('PTRATIO')
    plt.ylabel('HOME_VALUE')
    plt.show()
    X_ptratio = df['PTRATIO'].values
...
def exercise07():
    ...
    prediction_space = new_x.reshape((-1, 1))
