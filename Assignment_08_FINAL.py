...
class Iris:
    ...
    def __create(self):
        ...
        mycursor.execute("DROP DATABASE IF EXISTS "+self.__dbname)
        mycursor.execute("CREATE DATABASE "+self.__dbname)
        self.__conn = self.__get_connection(creds)
        mycursor = self.__conn.cursor()
    ...
    def display_gt(self,n): 
        # ------ Place code below here \/ \/ \/ ------
        self.__conn = self.__get_connection(creds)
        display = pd.read_sql("select * from Iris where id > "+str(n), con = self.__conn)
        self.close()
        print(display)
    ...
    def update_observation(self,id,new_target_species,new_target_species_id):
        ...
        sql_select_Query = "UPDATE Iris SET target_species = '"+str(new_target_species)+"' ,target_species_id = " + str(new_target_species_id)+" WHERE id = "+ str(id)
    ...
    def __get_connection(self,creds):
        return mc.connect(user=creds['user'], password=creds['password'],
                              host='127.0.0.1',
                              auth_plugin='mysql_native_password',database =self.__dbname)     
